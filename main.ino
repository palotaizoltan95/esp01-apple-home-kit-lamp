...
void cha_switch_on_setter(const homekit_value_t value) {
	...
	LOG_D("cha_switch_on_setter: %s", on ? "ON" : "OFF");
  if(!on) {
...
void set_bright(const homekit_value_t v) {
    current_brightness = v.int_value;
...
void update() {
  int analog_brightness = 0;
  if(current_brightness > 0) {
      analog_brightness = map(current_brightness,0, 100,0, 255);
  }
    
  LOG_D("update: %d", current_brightness);
  LOG_D("update: %d", analog_brightness);

  analogWrite(PIN_SWITCH, analog_brightness);
}
